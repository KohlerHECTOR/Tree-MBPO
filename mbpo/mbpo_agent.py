...
class MBPOAgent:
    def __init__(
        ...
        assert (
            nb_pol_update_per_optim <= self.nb_rollouts
        ), "nb pol update?optim sould be <= nb model rollout"
    ...
    def add_new_transi(self, s: np.ndarray):
        ...
        self.Term = np.concatenate(
            (self.Term, np.array(term, dtype=np.int8).reshape(-1, 1))
        )
