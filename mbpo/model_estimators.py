...
class FullTransitionModel:
    ...
    def fit(self, S: np.ndarray, A: np.ndarray, R: np.ndarray, Snext: np.ndarray):
        self.model.fit(
            np.concatenate((S, A), axis=1), np.concatenate((R, Snext), axis=1)
        )
...
class FullTransitionTreeCVModel(FullTransitionModel):
    def __init__(self):
        super().__init__(
            ...
            model_kwargs={
                ...
                "param_grid": {
                    "max_leaf_nodes": 2 ** np.arange(11, 18),
                    "criterion": ["friedman_mse"],
                },
                "n_jobs": 6,
            },
    ...
    def fit(self, S: np.ndarray, A: np.ndarray, R: np.ndarray, Snext: np.ndarray):
        ...
        print(
            "Model score: {}, Tree leaves: {}".format(
                self.cv.best_score_, self.cv.best_params_
            )
        )
...
class DoneTreeCVModel(DoneModel):
    ...
    def fit(
        ...
        print(
            "Done Model score: {}, Tree leaves: {}".format(
                self.cv.best_score_, self.cv.best_params_
            )
        )
        self.be = deepcopy(self.cv.best_estimator_)
...
class TransitionMLPModel(TransitionModel):
    def __init__(self):
        super().__init__(
            model=MLPRegressor,
            model_kwargs={"hidden_layer_sizes": [64, 64], "max_iter": 64},
        )
